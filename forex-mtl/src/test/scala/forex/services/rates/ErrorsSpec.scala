...
class ErrorsSpec extends FunSuite {
  ...
  test("combine should return base error when only base fails") {
    ...
    val quoteSuccess = Right(Some(PivotRate(forex.domain.currency.Currency.USD, forex.domain.rates.Price(BigDecimal(1.0)), forex.domain.rates.Timestamp(java.time.Instant.now()))))
  ...
  test("combine should return quote error when only quote fails") {
    val baseSuccess = Right(Some(PivotRate(forex.domain.currency.Currency.USD, forex.domain.rates.Price(BigDecimal(1.0)), forex.domain.rates.Timestamp(java.time.Instant.now()))))
  ...
  test("combine should handle UpstreamUnavailable with different services") {
    val baseError = Left(AppError.UpstreamUnavailable("ServiceA", "service A error"))
    val quoteError = Left(AppError.UpstreamUnavailable("ServiceB", "service B error"))
    ...
    result match {
      case AppError.UpstreamUnavailable(_, message) =>
        assert(message.contains("service A error"))
        assert(message.contains("service B error"))
      case _ => fail("Expected UpstreamUnavailable error")
    }
  }

  test("combine should handle empty error messages") {
    val baseError = Left(AppError.UpstreamUnavailable("CacheService", ""))
    val quoteError = Left(AppError.UpstreamUnavailable("CacheService", ""))
  ...
  test("combine should handle special characters in error messages") {
    val baseError = Left(AppError.UpstreamUnavailable("CacheService", "error with @#$% chars"))
    val quoteError = Left(AppError.UpstreamUnavailable("CacheService", "another @#$% error"))
